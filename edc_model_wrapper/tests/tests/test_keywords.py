...
class TestKeywords(TestCase):
    def test_url_parser(self):
        ...
        self.assertEqual(parse.urlencode(
            keywords, encoding="utf-8"), "a=1&b=2")

    def test_url_parser_included_keys1(self):
        obj = DummyObj(a=1, b=2)
        keywords = Keywords(objects=[obj], attrs=[
                            "a", "b"], include_attrs=["a", "b"])
        self.assertEqual(parse.urlencode(
            keywords, encoding="utf-8"), "a=1&b=2")
    ...
    def test_url_parser_included_keys3(self):
        ...
        self.assertEqual(parse.urlencode(
            keywords, encoding="utf-8"), f"a=1&b=&id={pk}")
    ...
    def test_url_parser_model(self):
        ...
        self.assertEqual(
            parse.urlencode(
                keywords, encoding="utf-8"), f"example={str(example.pk)}"
        )
